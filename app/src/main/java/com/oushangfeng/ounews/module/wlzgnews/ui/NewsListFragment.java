package com.oushangfeng.ounews.module.wlzgnews.ui;
...
import com.oushangfeng.ounews.bean.wlzgnews.News;
...
import com.oushangfeng.ounews.module.wlzgnews.presenter.INewsListPresenter;
import com.oushangfeng.ounews.module.wlzgnews.presenter.INewsListPresenterImpl;
import com.oushangfeng.ounews.module.wlzgnews.view.INewsListView;
...
public class NewsListFragment extends BaseFragment<INewsListPresenter> implements INewsListView {
    ...
    private BaseRecyclerAdapter<News> mAdapter;
    ...
    @Override
    public void updateNewsList(final RemoteResult data, @DataLoadType.DataLoadTypeChecker int type) {
        switch (type) {
            case DataLoadType.TYPE_REFRESH_SUCCESS:
                mRefreshLayout.refreshFinish();
                if (mAdapter == null) {
                    initNewsList(data.getListNews().getNews());
                } else {
                    mAdapter.setData(data.getListNews().getNews());
                }
            ...
            case DataLoadType.TYPE_LOAD_MORE_SUCCESS:
                ...
                if (data == null || data.getListNews().getNews().size() == 0) {
                    mRecyclerView.notifyAllLoaded();
                    toast("全部加载完毕噜(☆＿☆)");
                } else {
                    mAdapter.addMoreData(data.getListNews().getNews());
                    mRecyclerView.notifyMoreLoaded();
                }
                break;
    ...
    private void initNewsList(final List<News> data) {
        // mAdapter为空肯定为第一次进入状态
        mAdapter = new BaseRecyclerAdapter<News>(getActivity(), data) {
            @Override
            ...
            @Override
            public void bindData(BaseRecyclerViewHolder holder, int position, News item) {

                Glide.with(getActivity()).load(item.getImage_url()).asBitmap().animate(R.anim.image_load)
                ...
                holder.getTextView(R.id.tv_news_summary_title).setText(item.getTitle());
                holder.getTextView(R.id.tv_news_summary_digest).setText(item.getSummary());
                holder.getTextView(R.id.tv_news_summary_ptime).setText(item.getDate());
            }
        };
        ...
        mAdapter.setOnItemClickListener(new OnItemClickAdapter() {
            ...
            public void onItemClick(View view, int position) {
                ...
                KLog.e(mAdapter.getData().get(position).getTitle() + ";" + mAdapter.getData()
                        .get(position).getUrl());

                view = view.findViewById(R.id.iv_news_summary_photo);

                if (mAdapter.getData().get(position).getUrl() == 0) {
                    toast("此新闻浏览不了哎╮(╯Д╰)╭");
                    return;
                } else {
                    News item = mAdapter.getData().get(position);
                    toast(item.getUrl() + ":" + item.getSummary());
                }
